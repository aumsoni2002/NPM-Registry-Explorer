...
export default function HomePage() {
  ...
  const renderedPackages = featuredPackages.map((eachPackage) => {
    return (
      <div
      ...
      >
        <div className="flex flex-col gap-1 border-bottom border-gray-400">
          <div className="font-bold text-center">{eachPackage.name}</div>
          <div className="text-sm text-gray-500">{eachPackage.description}</div>
          <div className="text-sm text-gray-500">
            {eachPackage.maintainers.length} Maintainers
          </div>
        </div>
        <Link
          to={`/packages/${eachPackage.name}`}
